...
const formatPolygonData = (data) => {
  if (data[0].districts.length) {
    const intermediateArray1 = data[0].districts.map((innerObject) => {
...
export const apiDataSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getFloodData.fulfilled, (state, action) => {
        const formattedFloodData = formatFloodData(action.payload)
